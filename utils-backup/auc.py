...
def get_error(pred, true):
    ...
    for i in range(n_bootstraps):
        ...
        score0 = roc_auc_score(true[0,indices0], pred[0,indices0])
        score1 = roc_auc_score(true[1,indices1], pred[1,indices1])
        score2 = roc_auc_score(true[2,indices2], pred[2,indices2])
...
def get_auc(path, predictionr, labels, classes=[0, 1, 2]):
    ...
    if len(classes) > 2:
        ...
        roc_auc["cu0"]=get_error(predictions,labels)[1]
        roc_auc["cu1"]=get_error(predictions,labels)[3]
        roc_auc["cu2"]=get_error(predictions,labels)[5]
   
        roc_auc["cl0"]=get_error(predictions,labels )[0]
        roc_auc["cl1"]=get_error(predictions,labels) [2]
        roc_auc["cl2"]=get_error(predictions,labels)[4]
   
        ### Micro AUC ###
        fpr["micro"], tpr["micro"], _ = roc_curve(labels.ravel(), predictions.ravel())
        roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])
        roc_auc["micro_cl"]=get_error(predictions,labels)[8]
        roc_auc["micro_cu"]=get_error(predictions,labels)[9]
        ...
        roc_auc["macro_cl"]=get_error(predictions,labels)[6]      
        roc_auc["macro_cu"]=get_error(predictions,labels)[7]
